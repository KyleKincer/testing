...
Function test_assertion_with_complex_objects($t : cs:C1710.Testing)
	...
	$t.assert.areEqual($mockTest; $obj1.nested.value; $obj2.nested.value; "Different nested values should fail")
	$t.assert.isTrue($t; $mockTest.failed; "Different nested object values should fail the test")
	
	// Test same values pass
	$mockTest:=cs:C1710.Testing.new()  // Reset
	$obj2.nested.value:=42
	$t.assert.areEqual($mockTest; $obj1.nested.value; $obj2.nested.value; "Same nested values should pass")
	$t.assert.isFalse($t; $mockTest.failed; "Same nested object values should pass the test")
...
Function test_assertion_with_large_data($t : cs:C1710.Testing)
	...
	$t.assert.areEqual($mockTest; 1000; $largeCollection.length; "Large collection should have correct size")
	$t.assert.isFalse($t; $mockTest.failed; "Large data assertion should succeed")
...
Function test_log_message_overflow($t : cs:C1710.Testing)
	...
	$t.assert.areEqual($t; 100; $testing.logMessages.length; "Should store all log messages")
	$t.assert.areEqual($t; "Message 1"; $testing.logMessages[0]; "Should store first message correctly")
	$t.assert.areEqual($t; "Message 100"; $testing.logMessages[99]; "Should store last message correctly")
...
Function test_very_long_log_messages($t : cs:C1710.Testing)
	...
	$t.assert.areEqual($t; 1; $testing.logMessages.length; "Should store long message")
	$t.assert.areEqual($t; $longMessage; $testing.logMessages[0]; "Should store complete long message")
...
Function test_pattern_edge_cases($t : cs:C1710.Testing)
	...
	$t.assert.isFalse($t; $runner._matchesPattern(""; "test"); "Empty string should not match non-empty pattern")
	
	// Test patterns with special characters
	$t.assert.isTrue($t; $runner._matchesPattern("a*b"; "a@b"); "Should handle literal @ in 4D pattern")
	$t.assert.isTrue($t; $runner._matchesPattern("test_method"; "*_*"); "Should match patterns with underscores")
...
Function test_class_discovery_edge_cases($t : cs:C1710.Testing)
	...
	For each ($class; $testClasses)
		$t.assert.isTrue($t; $class.name="@Test"; "All test classes should end with 'Test', found: "+$class.name)
		$t.assert.isNotNull($t; $class; "Discovered class should not be null")
	End for each 
	
	// Should find at least the test classes we created
	$t.assert.isTrue($t; $testClasses.length>=4; "Should discover multiple test classes")
...
Function test_timing_precision($t : cs:C1710.Testing)
	...
	$t.assert.isTrue($t; $result.duration>=0; "Duration should be non-negative even for fast tests")
...
Function test_concurrent_test_contexts($t : cs:C1710.Testing)
	...
	$t.assert.isTrue($t; $test1.failed; "Test1 should be failed")
	$t.assert.isFalse($t; $test2.failed; "Test2 should not be failed")
	$t.assert.areEqual($t; 1; $test1.logMessages.length; "Test1 should have its message")
	$t.assert.areEqual($t; 1; $test2.logMessages.length; "Test2 should have its message")
	$t.assert.areEqual($t; "Message from test1"; $test1.logMessages[0]; "Test1 message should be correct")
	$t.assert.areEqual($t; "Message from test2"; $test2.logMessages[0]; "Test2 message should be correct")
...
Function test_memory_and_cleanup($t : cs:C1710.Testing)
	...
	For each ($suite; $runner.testSuites)
		...
		$t.assert.isNotNull($t; $suite.testFunctions; "Suite should have testFunctions collection")
	End for each